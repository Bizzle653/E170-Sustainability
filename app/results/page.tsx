  ...
  return <main className="resultsPage">
    ...
    <section className="metricGrid">
      ...
      <Metric
        ...
        value={pct(portfolio.maximum_drawdown)}
        note="Worst historical drop"
        hint={`The biggest fall from a peak to a low point over the past 3 years — the worst-case dip you'd have lived through. For your ${money(portfolio.total_investment_amount)}, that would be a drop to about ${money(portfolio.total_investment_amount * (1 + portfolio.maximum_drawdown))} before recovering.`}
        compare={portfolio.benchmark && `S&P 500: ${pct(portfolio.benchmark.maximum_drawdown)}`}
      />
      <Metric label="Holdings" value={String(portfolio.number_of_holdings)} note="2% minimum position" />
      <Metric label="Diversification" value={`${portfolio.diversification_score}/100`} note="Spread across sectors" hint="Higher means your money is spread across more industries and isn't quietly concentrated in holdings that move together — a form of risk reduction." />
    </section>
